...
for tool in docker jq grep find; do command -v "$tool" >/dev/null 2>&1 || fail "Missing $tool"; done
for document in README.md USER_DOC.md DEV_DOC.md; do
  [ -s "$document" ] || fail "Missing or empty $document"
done
[ "$(head -n 1 README.md)" = '# Inception' ] || fail 'README must start with the project title'
