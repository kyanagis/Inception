...
[ "$FTP_PASV_MIN_PORT" -ge 1024 ] && [ "$FTP_PASV_MAX_PORT" -le 65535 ] && [ "$FTP_PASV_MIN_PORT" -le "$FTP_PASV_MAX_PORT" ] || { echo "Invalid passive port range" >&2; exit 2; }
password=$(read_secret /run/secrets/ftp_password)
[ "${#password}" -eq 64 ] && printf '%s\n' "$password" | LC_ALL=C grep -Eq '^[0-9a-fA-F]{64}$' || { echo "Invalid FTP secret" >&2; exit 2; }
password_hash=$(printf '%s' "$password" | openssl passwd -6 -stdin)
unset password

install -d -m 0700 /run/proftpd
printf '%s:%s:33:33:WordPress files:/srv/wordpress/html:/usr/sbin/nologin\n' \
